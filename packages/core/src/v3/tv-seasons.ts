...
export const createV3TVSeasonsMethods = (
  client: Http,
  apiKey: string,
  url: string
): ITMDBAPI["v3"]["tvSeasons"] => {
  return {
    getDetails: async (tvId, seasonNumber, params) => {
      const res = await client.get<TVSeasonsGetDetailsResponse>(
        buildV3Url(apiKey, `${url}tv/${tvId}/season/${seasonNumber}`, params)
      );

      return res;
    },
    getAccountStates: async (tvId, seasonNumber, params) => {
      const res = await client.get<TVSeasonsGetAccountStatesResponse>(
        buildV3Url(
          apiKey,
    ...
    getAggregateCredits: async (tvId, seasonNumber, params) => {
      const res = await client.get<TVSeasonsGetAggregateCreditsResponse>(
        buildV3Url(
          apiKey,
    ...
    getChanges: async (seasonId, params) => {
      const res = await client.get<TVSeasonsGetChangesResponse>(
        buildV3Url(apiKey, `${url}tv/season/${seasonId}/changes`, params)
      );

      return res;
    },
    getCredits: async (tvId, seasonNumber, params) => {
      const res = await client.get<TVSeasonsGetCreditsResponse>(
        buildV3Url(
          apiKey,
    ...
    getExternalIds: async (tvId, seasonNumber, params) => {
      const res = await client.get<TVSeasonsGetExternalIDsResponse>(
        buildV3Url(
          apiKey,
    ...
    getImages: async (tvId, seasonNumber, params) => {
      const res = await client.get<TVSeasonsGetImagesResponse>(
        buildV3Url(
          apiKey,
    ...
    getTranslations: async (tvId, seasonNumber) => {
      const res = await client.get<TVSeasonsGetTranslationsResponse>(
        buildV3Url(
          apiKey,
    ...
    getVideos: async (tvId, seasonNumber, params) => {
      const res = await client.get<TVSeasonsGetVideosResponse>(
        buildV3Url(
          apiKey,
